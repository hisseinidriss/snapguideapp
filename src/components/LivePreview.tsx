...
import { functions } from "@/services/backend";
  ...
  return (
    <div className="flex-1 flex flex-col relative bg-muted/20 overflow-hidden">
      ...
      <div className="flex-1 relative overflow-auto">
        ...
        {/* Embed-style tour overlay */}
        {previewActive && step && (
          <div className="absolute inset-0 z-10">
            {/* Semi-transparent backdrop like the real embed */}
            <div
              className="absolute inset-0"
              style={{ background: "rgba(0,0,0,0.5)", transition: "opacity 0.3s" }}
              onClick={onClose}
            />

            {/* Tooltip styled to match the embed script output */}
            <div className="absolute inset-0 flex items-center justify-center p-6">
              ...
              >
                <h3 style={{ margin: "0 0 8px", fontSize: "16px", fontWeight: 600, color: "#000" }}>
                  {step.title}
                </h3>
                ...
                {step.selector && (
                  <div style={{
                    ...
                    <code style={{ fontSize: "11px", fontFamily: "monospace", color: "#888", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {step.selector}
                    </code>
                  </div>
                )}

                {(step as any).target_url && (
                  <div style={{
                    background: "#f5f5f5",
                    borderRadius: "6px",
                    padding: "6px 10px",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "12px", color: "#999" }}>
                    {previewStepIndex + 1} of {steps.length}
                  </span>
